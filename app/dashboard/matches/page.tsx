...
export default function MatchesPage() {
  ...
  const requestSession = async (teacherId: string, peerName: string) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        toast.error(data.error || 'Failed to request session');
        return;
      }
      toast.success(`Session requested with ${peerName}! They\'ll see it in their Sessions tab.`);
            ...
            return (
              <GlassCard key={`${match.username}-${match.offered_skill}-${idx}`} hover gradient className="relative overflow-hidden">
                ...
                <GradientButton
                  className="w-full"
                  size="sm"
                  onClick={() => requestSession(match.peer_id, match.username)}
                  disabled={requestingId === match.peer_id}
                >
                  {requestingId === match.peer_id ? (
